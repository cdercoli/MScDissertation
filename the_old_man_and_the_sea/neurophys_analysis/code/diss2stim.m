...
dataDissSubfolder = "./dissimilarity_analysis/";
...
for tr = 1:size(stim.data, 2)
    ...
    stim.names{10} = 'env+onset+BERT+CLIP+word2vec';
    stim.data{10,tr} = [env, x, xDiss, clipDiss, BrodDiss];
    
    stim.names{11} = 'env+onset+BERT+CLIP';
    stim.data{11,tr} = [env, x, xDiss,clipDiss];

    stim.names{12} = 'env+onset+BERT+brod';
    stim.data{12,tr} = [env, x, xDiss, BrodDiss];

    stim.names{13} = 'env+onset+BERT+envDerivative';
    stim.data{13,tr} = [env, x, xDiss, envDeriv];

    stim.names{14} = 'env+onset+BERT+brod+CLIP+envDerivative';
    stim.data{14,tr} = [env, x, xDiss, BrodDiss, clipDiss, envDeriv];

    stim.names{15} = 'env+onset+BERT+CLIP+envDerivative';
    stim.data{15,tr} = [env, x, xDiss, clipDiss, envDeriv];

    stim.names{16} = 'env+onset+BERT+brod+envDerivative';
    stim.data{16,tr} = [env, x, xDiss, BrodDiss, envDeriv];
